...
def criteria(
    ...
    **kwargs,
    ...
    Args:
        ...
        **kwargs: Additional parameters (provider config injected by Evaluation)
    """
    ...
    class CriteriaBasedMetric(BaseMetric):
        ...
        def __init__(self, **init_kwargs):
            ...
            # TODO: update this to use LLMProviderConfig once SimpleLLMHelper supports it.
            self.llm_config: OpenAIProviderConfig | None = init_kwargs.get("llm_config")
            self._llm_helper = None  # Initialized lazily with config
            self.execute_via = execute_via
            self.provider = "custom"

        @property
        def llm_helper(self) -> SimpleLLMHelper | None:
            if self.llm_config and self._llm_helper is None:
                self._llm_helper = SimpleLLMHelper(
                    self.llm_config, chat_model=self.llm_config.chat_model
                )
            return self._llm_helper

        @llm_helper.setter
        def llm_helper(self, value: SimpleLLMHelper):
            self._llm_helper = value
        ...
        def evaluate(self, sample: Any, **kwargs: Any) -> MetricResult:
            ...
            try:
                logger.debug(
                    f"Calling LLM for criteria metric {self.name}, model={self.llm_config.chat_model}"
                )
